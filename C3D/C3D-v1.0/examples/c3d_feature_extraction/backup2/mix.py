        ...
        for file2 in os.listdir(video_directory +'/'+ file):
            ...
            for file3 in os.listdir(each_video_path):
                ...
                if(seconds<30 and minute is 0):
                    empty_path = '/home/callbarian/C3D/empty_frame/001_resize.jpg'
                    concat_path = video_directory + '/' + name + '/concat_out.mp4'
                    result = subprocess.Popen(['ffmpeg', '-i', file_path, '-loop', '1', '-framerate', '25', '-t', '30', '-i', empty_path, '-filter_complex', "[0]scale=320x240,setsar=1[im];[1]scale=320x240,setsar=1[vid];[im][vid]concat=n=2:v=1:a=0", 'output_concat.mp4'],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
                    (out,err) = result.communicate()
                    result.wait()
